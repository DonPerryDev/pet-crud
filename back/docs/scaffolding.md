...
data class Pet(
    ...
    val deletedAt: LocalDateTime? = null  // For soft-delete support
)
```
...
interface {Entity}PersistenceGateway {
    ...
    fun update({entity}: {Entity}): Mono<{Entity}>
...
**Example (ListPetsUseCase for query operations):**
```kotlin
...
class {Entity}PersistenceAdapter(
    ...
    override fun update({entity}: {Entity}): Mono<{Entity}> =
        {entity}Repository.save({Entity}Mapper.toEntity({entity}))
            .map { {Entity}Mapper.toModel(it) }
}
```
