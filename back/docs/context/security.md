...
- Business validation performed in use case layer (see `RegisterPetUseCase`, `UpdatePetUseCase`, `GenerateAvatarPresignedUrlUseCase`, `ConfirmAvatarUploadUseCase`)
- Validations:
...
- **Protected endpoints:**
  - `GET /api/pets` — Requires valid JWT token (`.authenticated()`)
  - `POST /api/pets` — Requires valid JWT token (`.authenticated()`)
  - `PUT /api/pets/{petId}` — Requires valid JWT token and pet ownership
  - `DELETE /api/pets/{petId}` — Requires valid JWT token and pet ownership
