...
**Migrations:**
- `db/migrations/V1__create_pets_table.sql` — Initial table creation
- `db/migrations/V2__add_deleted_at_column.sql` — Add soft-delete support

**Indexes:**
- `idx_pets_owner_active` — Partial index on owner WHERE deleted_at IS NULL (for active pet queries)
- `idx_pets_id_active` — Partial index on id WHERE deleted_at IS NULL (for active pet lookups)
...
- `V2__add_deleted_at_column.sql` — Add deleted_at TIMESTAMP column and partial indexes for soft-delete support

Migration strategy: SQL scripts in `db/migrations/` directory (convention-based versioning managed by Flyway).

**Soft-Delete Pattern:**
- All queries filter by `deleted_at IS NULL` to exclude deleted pets
- PetRepository custom queries include this filter
- findById, findAllByOwner, countByOwner all exclude soft-deleted records
