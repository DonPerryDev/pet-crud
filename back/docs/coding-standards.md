...
### Conditional Guards with `filter` + `switchIfEmpty`

When a reactive chain needs to reject an element based on a condition (e.g. ownership check, status validation), prefer `filter` + `switchIfEmpty(Mono.defer { Mono.error(...) })` over `flatMap` with imperative `if`/`else` returning `Mono.error` or `Mono.just`.

```kotlin
// CORRECT: Declarative guard — filter + switchIfEmpty
.filter { it.owner == command.userId }
.switchIfEmpty(Mono.defer {
    logger.warning("[${command.petId}] Unauthorized attempt")
    Mono.error(UnauthorizedException("Not authorized"))
})

// WRONG: Imperative branching inside flatMap
.flatMap { entity ->
    if (entity.owner != command.userId) {
        Mono.error(UnauthorizedException("Not authorized"))
    } else {
        Mono.just(entity)
    }
...
Use `Mono.defer` inside `switchIfEmpty` when you need side effects (logging) or lazy error construction. If no side effects are needed, `Mono.error(...)` alone is sufficient.
